...
func (c *Controller) Attacking() {
	...
	if IsRayHit {
		blockID := Map.TileID(targetBlockPos.X, targetBlockPos.Y)
		...
		if blockHealth >= 180 {
			blockHealth = 0
			Map.SetTileID(targetBlockPos.X, targetBlockPos.Y, items.Air)
			...
			// spawn drop item
			x, y := Map.TileToWorldCenter(targetBlockPos.X, targetBlockPos.Y)
