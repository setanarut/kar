...
var dropItemFilterWithPlayer = cm.ShapeFilter{
	...
	Mask:       cm.AllCategories &^ res.PlayerRayMask &^ res.DropItemMask,
}
...
func (ps *Physics) Init() {
	...
	playerDropItemHandler := res.Space.NewCollisionHandler(res.CollPlayer,
		res.CollDropItem)
	DropItemBlockHandler := res.Space.NewCollisionHandler(res.CollDropItem,
		res.CollBlock)
